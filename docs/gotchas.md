...
- **Hook git discovery**: All hook commands in `.claude/settings.json` use `GIT_DISCOVERY_ACROSS_FILESYSTEM=1 git rev-parse --show-toplevel` to locate the repo root. Without this, hooks fail with `fatal: not a git repository` when Claude is opened from a directory outside the repo (e.g., home dir). Gotcha: if Claude is ever opened from inside a *different* git repo on the same volume, the hook resolves to that repo's root instead of tokencast — scripts won't be found and hooks will error.
