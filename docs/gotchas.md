...
- **`${CLAUDE_PLUGIN_ROOT}` expansion**: The variable is used in `plugin/hooks/hooks.json` but its expansion behavior in Claude Code's plugin hook system is unverified at runtime — must be confirmed after first install. If it fails, consider wrapper scripts using `$(dirname "$0")` as fallback.
