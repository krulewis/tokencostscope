...
- **`estimate_cost` $0.00 bug (FIXED in PR #31)**: Previously, `_resolve_steps()` checked raw strings against `PIPELINE_STEPS` keys without resolving aliases. Passing `"qa"` instead of `"QA"` would silently drop steps. Now `resolve_step_name()` is called before the membership check, resolving aliases like `"test-writing"` → `"Test Writing"` and `"qa"` → `"QA"`.
...
- **EWMA bias for infrequent users**: The current formula `result = alpha * (v * w) + (1 - alpha) * result` biases factors below the true ratio when decay weights w < 1.0. Old records with small w contribute `alpha * v * w` instead of `alpha * v`, moving the result toward a deflated target. Normalizing weights by max_w alone does NOT fix this — even with max_w=1.0, 19 of 20 records still have w < 1.0 and each deflates the result. The correct fix is to modulate alpha: `result = (alpha * w) * v + (1 - alpha * w) * result`. With this formula, old records barely move the result (tiny effective alpha); recent records update normally (full alpha). For any constant sequence v=k, the EWMA converges exactly to k regardless of weights.
