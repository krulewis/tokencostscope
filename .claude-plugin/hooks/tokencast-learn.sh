...
CALIBRATION_DIR="${HOME}/.tokencast/calibration"
...
if [ ! -f "$ESTIMATE_FILE" ]; then
    ...
    if [ -f "$LAST_ESTIMATE_MD" ]; then
        ...
        python3 "$SCRIPTS_DIR/parse_last_estimate.py" "$LAST_ESTIMATE_MD" > "$ESTIMATE_FILE" 2>/dev/null || {
            rm -f "$ESTIMATE_FILE"
            exit 0
        }
...
ACTUAL_JSON=$(python3 "$SCRIPTS_DIR/sum-session-tokens.py" "$LATEST_JSONL" "$BASELINE_COST" \
...
_sr_path = os.path.join(script_dir, 'session_recorder.py')
...
"
    ...
    python3 "$SCRIPTS_DIR/calibration_store.py" append-history \
