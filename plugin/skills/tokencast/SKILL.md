...
Read `~/.tokencast/calibration/factors.json` if it exists → learned correction factors from prior runs.
...
Read `step_factors` and `signature_factors` from `~/.tokencast/calibration/factors.json` if it
...
```
Read ~/.tokencast/calibration/last-estimate.md if it exists → prior expected_cost for delta comparison.
Read ~/.tokencast/calibration/active-estimate.json if it exists → structured prior estimate data.
...
```
Write ~/.tokencast/calibration/active-estimate.json:
{
...
```
Write ~/.tokencast/calibration/last-estimate.md:
...
/usr/bin/python3 ../../scripts/tokencast-status.py \
    [--history ~/.tokencast/calibration/history.jsonl] \
    [--factors ~/.tokencast/calibration/factors.json] \
    [--heuristics ../../skills/tokencast/references/heuristics.md] \
